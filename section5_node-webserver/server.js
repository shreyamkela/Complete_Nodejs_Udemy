...
// We want to serve all contents in the public folder without having to set the path by different handlers in express, for each file in public folder. This can be done by using express middleware. WIth it we can add the path to a folder containing html files, css files, images, etc and that sets up the single handler that serves all the files in that folder
